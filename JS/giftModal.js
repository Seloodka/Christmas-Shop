import { cardsArray } from './globalVars.js';

const cloneCardImage = (card) => {
...
const getCardDataFromName = (name) => {
  return cardsArray.find((card) => card.name === name);
}

const createModalTextTemplate = (cardData) => {
  return `
  <span class="card-tag">${cardData.category}</span>
...
const createStatRatingSnowflakes = (rating) => {
  const snowflakes = [];
...
const createModalSuperpowerTemplate = (superpowers) => {

  let html = ` 
  <div class="card-stats">
    <span class="stats-title">Adds superpowers to:</span>
    <div class="stats-wrapper">`;
  
  for (let power in superpowers) {
    const powerRating = parseInt(superpowers[power]) / 100;
    
    html += ` 
    <div class="stat">
      <span class="stat-name">${power}</span>
      <div class="stat-rating-wrapper">
        <span class="rating-amount">${superpowers[power]}</span>
        <div class="rating-container">
        ${createStatRatingSnowflakes(powerRating).join("")}
         </div>
      </div>
    </div>`;
  }
...
const cardClickHandler = (event) => {
  ...
  const cardData = getCardDataFromName(cardName);

  const modalText = createModalTextTemplate(cardData);
  const modalSuperpowers = createModalSuperpowerTemplate(cardData.superpowers);


  modalContent.imageContainer.append(cloneCardImage(card));
  modalContent.contentWrapper.insertAdjacentHTML("afterbegin", modalText);
  modalContent.contentWrapper.insertAdjacentHTML("beforeend", modalSuperpowers);
...
const modalContent = {
  imageContainer: modal.querySelector(".image-container"),
  contentWrapper: modal.querySelector(".modal-text-wrapper"),
}
